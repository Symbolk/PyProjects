...
def predict_vars(predict_file_path, var_model_path, output_path, raw_var_path, encoded_raw_var):
    ...
    encoded_pred_var = predict_file_path[0:-7]+'var_encoded.csv'
    ...
    if_ids = predict_data_values[:, 0:1]
    var_names = predict_data_values[:, 5:6]
    locators = np.concatenate((if_ids, var_names), axis=1)
    row_num = list()
    raw_data = pd.read_csv(raw_var_path, sep='\t', header=0)
    raw_data_values = raw_data.values
    raw_locators = np.concatenate((raw_data_values[:,0:1], raw_data_values[:,5:6]),axis=1)
    for l in locators:
    ...
    for r in row_num:
        ...
        if_ids.append(raw_data_values[r: 0])
        varnames.append(raw_data_values[r, 5])
    print(row_encoded)
